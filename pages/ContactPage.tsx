  ...
  return (
    <div>
      <h1 className="font-heading text-5xl text-brand-accent uppercase tracking-wider mb-4 text-center">
        <Snarky normal="Get In Touch" snark="Summon Me" />
      </h1>
      <p className="text-center text-gray-400 max-w-2xl mx-auto mb-12">
        <Snarky
          normal="Have a project in mind, a question, or just want to say hi? I'd love to hear from you."
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="bg-brand-bg-800 p-8" style={{ clipPath: 'polygon(0 0, 100% 0, 100% calc(100% - 15px), calc(100% - 15px) 100%, 0 100%)' }}>
          <h2 className="font-heading text-2xl text-white mb-6">Send a Message</h2>
        ...
        <div className="flex flex-col justify-center space-y-6">
            <h3 className="font-heading text-2xl text-white">...Or Use a Carrier Pigeon</h3>
             <p className="text-gray-400">If forms aren't your thing, here are some faster ways to reach me.</p>
            <a href={`mailto:${profile.email}`}>
              <Button variant="outline" className="w-full"><Mail className="mr-3" />Email Me</Button>
            </a>
            <a href={`tel:${profile.phone}`}>
              <Button variant="outline" className="w-full"><Phone className="mr-3" />Call Me</Button>
            </a>
            <a href={`https://wa.me/${profile.phone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer">
                <Button variant="outline" className="w-full"><MessageSquare className="mr-3" />WhatsApp</Button>
            </a>
        </div>
      </div>
    </div>
